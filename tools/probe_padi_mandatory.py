...
4. What is left is the figure. **Every other boat that prices a visa prices it
   at 25-30** -- Blue 30, Freedom III 25 -- which is what an Egyptian visa
   costs. Seawolf's is 250, ten times that and inside the same range as its own
   package. The likeliest reading is an operator entering a package total
   against a visa-titled catalogue item, on PADI's side, and this probe cannot
   prove it.

So no line is dropped on the strength of this. A published mandatory charge is
not ours to delete on an inference, and "never invent a price" cuts both ways.
What the finding supports is refusing to call such a bill *complete*: a
disclosure that names one charge twice does not add up, which is the same
verdict this site already reaches for an unpriced mandatory line.
