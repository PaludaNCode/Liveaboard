...
| Is a trip's dive count ever genuinely absent rather than unparsed? | **Once, and the source says so.** Aphrodite's *North Dolphins* (tour 395645) prints `Dives  -` where all 316 other trips print a number, and it is the only dash in the book. Not a parse failure: its day plan is "2–3 guided **snorkeling** sessions daily", guests "prepare their snorkeling equipment", the stated bar is *No Certificate needed*, and PADI — which sells eleven Aphrodite itineraries — does not list this one. A snorkelling cruise has no dive count because it has no dives. Note the fragment still labels it `Trip type  Diving cruise`, so that field cannot be used to tell the two apart. | a live fetch, 2026-09-01 |
| Does the vessel page's **Highlights** block state a dive count? | **On 2 of 12 vessels probed, and neither is usable.** The block is free marketing prose (present on 8 of 12) — Aphrodite's reads "Tech Diving / Trimix support · ENOS system on board · Diving courses available · **Up to 20 dives + night dives**", and Emperor Asmaa's "Deep South trips that give 21 dives in a week". Both are *upper* bounds, which inverts the rule that the dataset keeps the low end so price per dive stays a ceiling; both are vessel-level, which is what `_dives`' `for_nights` guard exists to keep off other trip lengths. Taking Aphrodite's would put 20 dives on the snorkelling cruise above. **Not a source.** | a live probe, 12 vessels, 2026-09-01 |
| Can tour ids be had for a boat with no in-season departure? | **Yes — they are on the vessel page**, and this is why 85 itineraries have no fragment. `fetch_itineraries.py` takes its ids from `Event` nodes in `data/archive.json`, so a `padi_only` boat contributes none by construction. Fetched over plain `urllib`: **MY Blue Pearl 12 ids, Bella 2 four, Eriny one** — all three barren-listed and `padi_only` — and the fragments answer in full ("Approximately 18 dives in total", "Up to 20 guests"). Nine MY Blue Pearl itineraries publish today with no fragment at all. The join is the catch: `itinerary_key` keeps the port pair, and the two sources punctuate it differently — `North & Tiran (Hurghada-Hurghada)` against the fragment's `(Hurghada - Hurghada)`, so 2 of Blue Pearl's first 3 fail on whitespace alone. Key on the name plus `port_from`/`port_to`. See `docs/plan-missing.md` §2. | a live probe, 2026-09-01 |
| Do the two sellers spell the same trip the same way? | **No, and it is not only punctuation.** Done: the discovery pass above read 162 tour ids off 15 vessel pages and took the book from 317 trips to 352, but only 6 published itineraries joined on the port fix alone. A vessel liveaboard.com sells no berth on takes its trip *names* from PADI, so the fragment spells the same week differently -- *St. Johns* against *St. John's*, *Brothers, Daedalus, Elphinstone* against *Brothers, Daedalus & Elphinstone*. `padi_key`, which exists to look a foreign record up, is the fallback where `itinerary_key` misses, worth 5 more; where it names two of a boat's own trips (Emperor Asmaa lists *South & St Johns* and *South and St. Johns*) both are refused. Net 85 -> 74 unfragmented. | `promote`, all 402 itineraries |
| Should the harbour names be folded through `PORT_ALIASES` to join more? | **No -- measured and refused.** *Marsa Ghalib* and *Port Ghalib* are the same harbour and that table says so, which makes it look free. Folding both ends of the port pair into the key buys **one** extra match and creates **two** collisions, one of them Blue Horizon's own two itineraries onto a single key. A key that merges two of our own trips serves one's dive count and reefs for the other, which is worse than the miss it fixes. The spacing around the dash is punctuation and is normalised; the harbour is not. | `promote`, all 402 itineraries |
